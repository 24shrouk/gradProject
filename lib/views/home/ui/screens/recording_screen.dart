...
//     try {
//       final response =
//           await Dio().get('http://192.168.1.12/detect_topics/'); //10.0.2.2:8000
...
//     try {
//       final response = await Dio().get('http://192.168.1.12/extract_tasks/');
//       if (response.statusCode == 200) {
//         final data = response.data;
...
//     final dio = Dio();
//     const url =
//         'http://192.168.1.12/enhance/'; // غيّري IP لو بتجربي على جهاز حقيقي
...
//     final dio = Dio();
//     const url =
//         'http://192.168.1.12/summarize/'; // غيّري IP لو بتجربي على موبايل حقيقي
...
import 'package:gradprj/core/theming/my_colors.dart';
import 'package:gradprj/core/theming/my_fonts.dart';
import 'package:gradprj/views/home/ui/widgets/add_trello.dart';
import 'package:http_parser/http_parser.dart';
import 'package:path_provider/path_provider.dart';
...
class _RecordingScreenState extends State<RecordingScreen>
  ...
  Future<void> uploadAudio() async {
    ...
    try {
      ...
      Response response = await Dio().post(
        "http://192.168.1.12:8000/transcribe/",
        data: formData,
        options: Options(headers: {"Content-Type": "multipart/form-data"}),
      );
  ...
  Future<void> fetchTopics() async {
    ...
    try {
      final response =
          await Dio().get('http://192.168.1.12:8000/detect_topics/');
      if (response.statusCode == 200) {
        final data = response.data;
  ...
  Future<void> fetchTasks() async {
    ...
    try {
      final response =
          await Dio().get('http://192.168.1.12:8000/extract_tasks/');
      if (response.statusCode == 200) {
        final data = response.data;
        // تقسيم المهام إلى قائمة باستخدام new line
        extractedTasks = (data['tasks'] as String)
  ...
  Future<void> fetchEnhancedText() async {
    ...
    final dio = Dio();
    const url =
        'http://192.168.1.12:8000/enhance/'; // غيّري IP لو بتجربي على جهاز حقيقي
  ...
  Future<void> fetchSummary() async {
    ...
    final dio = Dio();
    const url =
        'http://192.168.1.12:8000/summarize/'; // غيّري IP لو بتجربي على موبايل حقيقي
